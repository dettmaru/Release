...
int NetworkConnect(Network* n, char* addr, int port, char* server_root_certificate) {
    char portbuf[100];
    int ret, flags;
	int retVal = -1;
	int len;
    ...
    ret = mbedtls_x509_crt_parse(&n->cacert, (uint8_t*)server_root_certificate, strlen(server_root_certificate)+1);
    if(ret < 0)
    {
