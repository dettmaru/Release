...
int8_t flash_space[4];
int8_t flash_space_temp[4];
flash_flags ff;
...
void cpy_arr_to_struct(){
  ...
  flash_data->sensor_init = (int8_t)flash_space[0];
  flash_data->sensor_name_number = (int8_t)flash_space[1];
  flash_data->tx_power = flash_space[2];
}
...
void cpy_struct_to_arr(){
  ...
  flash_space[0] = (uint8_t)flash_data->sensor_init;
  flash_space[1] = (uint8_t)flash_data->sensor_name_number;
  flash_space[2] = flash_data->tx_power;
...
void flash_init(){
  ...
  {
      #if(NRF_FLASH_LOG_ON)
        NRF_LOG_INFO("FDS_Init: SUCCESS.");
        NRF_LOG_FLUSH();
      #endif
  } else if(ret == FDS_ERR_NO_PAGES){
      #if(NRF_FLASH_LOG_ON)
        NRF_LOG_INFO("FDS_Init: there is no space available in flash memory to install the file system..");
        NRF_LOG_FLUSH();
      #endif
  }
}
 ...
 * @laste dit   04.01.2020
 *              
 * @brief	flash_delete_file()
 *              Files consists out of several records,
 *              this function delets the files.
 *
 * @param 
 * 		
 *              -
 */
void flash_delete_file(){


    fds_file_delete(FILE_ID_SENSOR_NAME);
    while(ff.flash_file_del_finish == 0);
    ff.flash_file_del_finish = 0;
